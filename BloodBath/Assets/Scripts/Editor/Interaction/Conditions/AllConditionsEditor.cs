...
public class AllConditionsEditor : Editor
{
	...
	private void OnEnable()
	{
		...
		if (allConditions.allConditions == null)
			allConditions.allConditions = new Condition[0];
	...
	private void CreateEditors ()
	{
		...
		conditionEditors = new ConditionEditor[allConditions.allConditions.Length];
	...
	private static void CreateAllConditionsAsset()
	{
		...
		instance.allConditions = new Condition[0];
	}
	...
	private void AddCondition(string description)
	{
		...
		ArrayUtility.Add(ref AllConditions.INSTANCE.allConditions, newCondition);
	...
	public static void RemoveCondition(Condition condition)
	{
		...
		ArrayUtility.Remove(ref AllConditions.INSTANCE.allConditions, condition);
	...
	public static int TryGetConditionIndex (Condition condition)
	{
		...
		for (int i = 0; i < TryGetConditionsLength (); i++)
		{
			...
			if (TryGetConditionAt (i).uid == condition.uid)
				return i;
		}
	...
	public static Condition TryGetConditionAt (int index)
	{
		...
		Condition[] allConditions = AllConditions.INSTANCE.allConditions;
	...
	public static int TryGetConditionsLength ()
	{
		...
		if (AllConditions.INSTANCE.allConditions == null)
			return 0;

		// Otherwise return the length of the array.
		return AllConditions.INSTANCE.allConditions.Length;
	}
}
